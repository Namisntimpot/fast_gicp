...
def test_cuda_vs_cpu_speedup(pg, n_target):
    ...
    cuda_min, cuda_mean, T_cuda = _time_align(cuda_align)

    rot_cpu, t_cpu = si.pose_error(T_gt, T_cpu.astype(np.float64))
    rot_cuda, t_cuda = si.pose_error(T_gt, T_cuda.astype(np.float64))
    speedup = cpu_min / max(cuda_min, 1e-9)
    ...
    out = dict(
        ...
        cuda_min_s=cuda_min, cuda_mean_s=cuda_mean,
        speedup_min=speedup,
        cpu_rot_deg=rot_cpu, cpu_trans_m=t_cpu,
        cuda_rot_deg=rot_cuda, cuda_trans_m=t_cuda,
    )
    with open(os.path.join(results_dir(), f"speedup_n{n_target}.json"), "w") as f:
        json.dump(out, f, indent=2)
    print(f"\n  [n_target={actual_target}, n_source={actual_source}]")
    print(f"    CPU  min/mean = {cpu_min*1000:.1f} / {cpu_mean*1000:.1f} ms   "
          f"rot={rot_cpu:.4f} deg  trans={t_cpu:.5f} m")
    print(f"    CUDA min/mean = {cuda_min*1000:.1f} / {cuda_mean*1000:.1f} ms  "
          f"rot={rot_cuda:.4f} deg  trans={t_cuda:.5f} m")
    print(f"    Speedup (best CPU / best CUDA): {speedup:.2f}x")

    # Sanity floors so a silent regression is caught:
    assert rot_cuda < 1.0, f"CUDA rotation diverged at n={n_target}"
    assert t_cuda < 0.10, f"CUDA translation diverged at n={n_target}"
...
def test_summary_plot(pg):
    ...
    cuda_ms = [c["cuda_min_s"] * 1000 for c in cases]
    speedup = [c["speedup_min"] for c in cases]

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(11, 4))
    ax1.plot(ns, cpu_ms, "o-", color="#1f77b4", label="CPU (16 threads)")
    ax1.plot(ns, cuda_ms, "s-", color="#2ca02c", label="CUDA")
    ax1.set_xlabel("target cloud size")
    ax1.set_ylabel("align() wall time (ms)")
    ax1.set_xscale("log")
    ax1.set_yscale("log")
    ax1.grid(True, which="both", alpha=0.3)
    ax1.legend()
    ax1.set_title("Wall time vs target size")
    for n, c, g in zip(ns, cpu_ms, cuda_ms):
        ax1.annotate(f"{c:.0f}ms", (n, c), textcoords="offset points",
                     xytext=(0, 6), ha="center", fontsize=7, color="#1f77b4")
        ax1.annotate(f"{g:.0f}ms", (n, g), textcoords="offset points",
                     xytext=(0, -14), ha="center", fontsize=7, color="#2ca02c")

    ax2.bar([str(n) for n in ns], speedup, color="#ff7f0e")
    ax2.set_xlabel("target cloud size")
    ax2.set_ylabel("speedup (CPU/CUDA, best-of-N)")
    ax2.axhline(1.0, color="#888", linestyle="--", linewidth=1)
    ax2.set_title("CUDA speedup")
    for i, v in enumerate(speedup):
        ax2.text(i, v, f"{v:.2f}x", ha="center", va="bottom", fontsize=8)
