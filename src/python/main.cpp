...
PYBIND11_MODULE(pygicp, m) {
  ...
  py::class_<LsqRegistration, std::shared_ptr<LsqRegistration>>(m, "LsqRegistration")
    ...
    .def("get_max_iterations", [] (LsqRegistration& reg) { return reg.getMaximumIterations(); })
    .def("set_transformation_epsilon", [] (LsqRegistration& reg, double eps) { reg.setTransformationEpsilon(eps); })
